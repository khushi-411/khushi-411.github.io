...
in each dimension, and T-2 be the output grid points.
Therefore, each block will have (T-2)^3 active threads calculating
the output grid point values. Therefore, the number of operations
performed is given by 13*(T-2)^3 (note 13 here is the number of
floating-point arithmetic operations). FLOPS is given by (13*(T-2)^3)/(4*T^3) OP/B.
...
overcome the work done by each thread to calculate one grid point value.
The aim is to serialize the parallel units of work into each thread and
reduce the price of parallelism. The main advantage is it increases the
tile size without increasing the number of threads. The other advantage
is it does not require all planes of the input tile to be present
in the shared memory. The kernel with thread coarsening in the z direction is given by:
```cuda
__global__ void stencil_kernel(float* in, float* out, unsigned int N) {
...
Only inCurr_s elements truly need to be in the shared memory;
others can stay in the registers of the single-user thread. Hence,
we take advantage of the register tiling technique. The computed
output tile is stored in the registers of the blocks thread.
Coarsening and register tiling kernels have two major advantages
...
- As mentioned earlier, each block consumes 1/3 of the shared memory. This will lead to higher register usage. Programmers should keep this trade-off in mind while writing the kernel.

...
### **Introduction**
Stencils are used to solve partial differential equations in applications
...
This blog post focuses on the computation pattern where the
...
Structured grids are mainly used for the finite difference method
to find the derivative of a variable and the unstructured
grids are used in finite-element and finite-volumne methods
...
the derivative of a 1 dimensional function *f(x)*, we discretized
...
for any change from input to the output.
...
in each dimension, so T-2 will be the output grid points.
Therefore, each block will have (T-2)<sub>3</sub> active threads calculating
the output grid point values. Therefore, the number of operations
performed is given by 13*(T-2)<sub>3</sub> (note 13 here is the number of
floating-point arithmetic operations). FLOPS is given by (13*(T-2)<sub>3</sub>)/(4*T<sub>3</sub>) OP/B.
...
overcome the work done by each thread to calculate from calculating
one grid point to a column grid point values.
...
- It increases the tile size without increasing the number of threads. But the number
of threads remains unchanged, which simplifies scheduling. For the original block size,
the thread block size would be T<sub>3</sub>. But for the coarsened block size,
the thread block size is reduced to T<sub>2</sub>.
- The other advantage
is it does not require all planes of the input tile to be present
in the shared memory. For example, when all the blocks are in shared memory,
it would require 3 * T<sub>3</sub> * 4 bytes of memory while for the coarsened
block will requre 3 * T<sub>2</sub> * 4 bytes of memory.
...
### **Register Tiling**
Previously, each inPrev_s and inNext_s element required only one
thread in the calculation of the output tile grid point.
Only inCurr_s elements are accessed by multiple threads
and hence it truly need to be in the shared memory;
others can stay in the registers of the single-user thread. Hence,
we take advantage of the register tiling along
with the thread coarsening technique. The computed
output tile is stored in the registers of the blocks thread.
...
- As mentioned earlier, each block consumes 1/3 of the shared memory. This will lead to higher register usage. Note that the programmers should keep this trade-off in mind while writing the kernel.

Also, note this register tiling with coarsening does not

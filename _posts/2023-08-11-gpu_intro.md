...
and 312 TFLOPS for FP16 datatypes, which are incredible results compared
...
<table align="center" style="margin: 0px auto;">
  ...
  <tr>
    <th><img alt="CPU Architecture" src="/assets/CUDA/cpu_arch.png" width="200" ></th>
    <th><img alt="GPU Architecture" src="/assets/CUDA/gpu_arch.png" width="200" ></th>
  </tr>
</table>
...
function in host memory (`vecAdd`) that calls `vecAddKernel` initiated
in the global memory (memory that can be written and read from both
...
__global__ void vecAddKernel(float* A, float* B, float* C, int n) {
    // local variable `i`, generated for each thread
...
void vecAdd(float* A_h, float* B_h, float* C_h, int n) {
    ...
    if (allocErr != cudaSuccess) {
        std::cerr << "Memory allocation for A_d failed: " << cudaGetErrorString(allocErr) << std::endl;
        return;
    }
    allocErr = cudaMalloc((void**)&B_d, size);
    if (allocErr != cudaSuccess) {
        std::cerr << "Memory allocation for B_d failed: " << cudaGetErrorString(allocErr) << std::endl;
        cudaFree(A_d);
        return;
    }
    allocErr = cudaMalloc((void**)&C_d, size);
    if (allocErr != cudaSuccess) {
        std::cerr << "Memory allocation for C_d failed: " << cudaGetErrorString(allocErr) << std::endl;
        cudaFree(A_d);
        cudaFree(B_d);
        return;
    }
...
[Intel's blog on Intrinsics](https://www.intel.com/content/www/us/en/docs/intrinsics-guide/index.html).
In the code below, `_mm_loadu_ps()`, `_mm_set_ps()`, `_mm_add_ps`, and `_mm_store_ps`
are built-in variables to which the compiler maps directly to vector instructions.
